    ...
    function window_loaded() {
        ...
        $( document.body ).on( 'click', '.porto-install-demo .message-section .install-plugin, .porto-install-demo .plugins-used .install-plugin', function ( e ) {
            ...
            if ( $this.closest( '.plugins-used' ).length ) {
                is_plugin = true;
                $plguin_wrap = $this.closest( '.plugins-used' );
            } else {
                $plguin_wrap = $this.closest( '.pagebuilder-selector' );
            }
            var builder = $plguin_wrap.find( 'input:checked' ).val();
                ...
                if ( typeof response == 'object' && typeof response.message != 'undefined' ) {
                    if ( typeof response.url != 'undefined' ) {
                        ...
                        if ( is_plugin ) {
                            ...
                            if ( !$plguin_wrap.find( '>li:not(.plugin-step)' ).length ) {
                                $plguin_wrap.siblings( '.porto-install-section' ).slideDown();
                                $plguin_wrap.remove();
                            }
                        } else {
                            if ( 'revslider' == slug || 'dynamic-featured-image' == slug || 'porto-vc-addon' == slug ) {
                                $plguin_wrap.find( '.message-section>.' + slug ).remove();
                            } else {
                                $plguin_wrap.find( '.message-section>.' + slug ).addClass( 'd-none' );
                            }

                            if ( 'elementor' == builder ) {
                                if ( $plguin_wrap.find( '.message-section>.elementor' ).hasClass( 'd-none' ) && ( 0 == $plguin_wrap.find( '.message-section>.dynamic-featured-image:not(.d-none)' ).length ) ) {
                                    $plguin_wrap.find( '>label.elementor' ).removeClass( 'notinstalled' );
                                    $import_action.slideDown();
                                }
                            } else if ( 'js_composer' == builder ) {
                                if ( ( 0 == $plguin_wrap.find( '.message-section>.revslider_j:not(.d-none)' ).length ) && $plguin_wrap.find( '.message-section>.js_composer' ).hasClass( 'd-none' ) && ( 0 == $plguin_wrap.find( '.message-section>.dynamic-featured-image:not(.d-none)' ).length ) ) {
                                    $plguin_wrap.find( '>label.js_composer' ).removeClass( 'notinstalled' );
                                    $import_action.slideDown();
                                }
                            } else if ( 'vc' == builder ) {
                                if ( $plguin_wrap.find( '.message-section>.vc' ).hasClass( 'd-none' ) && ( 0 == $plguin_wrap.find( '.message-section>.porto-vc-addon:not(.d-none)' ).length ) ) {
                                    $plguin_wrap.find( '>label.vc' ).removeClass( 'notinstalled' );
                                    $import_action.slideDown();
                                }
                            }
                        }

...
if ( ! empty( $porto_settings['product-sale'] ) || ( ! empty( $porto_settings['product-labels'] ) && in_array( 'sale', $porto_settings['product-labels'] ) ) ) {
	if ( $product->is_on_sale() ) {
		...
		if ( $reg_p ) {
			$percentage = - round( ( ( $reg_p - ( ! empty( $product->get_sale_price() ) ? floatval( $product->get_sale_price() ) : 0 ) ) / $reg_p ) * 100 );
		} elseif ( 'variable' == $product->get_type() && $product->get_variation_regular_price() ) {
			$percentage = - round( ( ( floatval( $product->get_variation_regular_price() ) - floatval( $product->get_variation_sale_price() ) ) / floatval( $product->get_variation_regular_price() ) ) * 100 );
		}

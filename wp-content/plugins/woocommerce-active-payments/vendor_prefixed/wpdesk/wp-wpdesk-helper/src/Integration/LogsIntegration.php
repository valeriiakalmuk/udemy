...
use ActivePaymentsVendor\WPDesk\Logger\WPDeskLoggerFactory;
use ActivePaymentsVendor\WPDesk\Notice\Notice;
 ...
 */
class LogsIntegration implements \ActivePaymentsVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    /** @var Logger */
    private $logger;
    public function __construct(\ActivePaymentsVendor\WPDesk\Helper\Page\SettingsPage $settings_page)
    {
    ...
    {
        \add_settings_section('wpdesk_helper_debug', \__('Debug', 'woocommerce_activepayments'), null, $this->settings_page->get_page_name());
        \add_settings_field(self::DEBUG_LOG_SETTING_KEY, \__('WP Desk Debug Log', 'woocommerce_activepayments'), function () {
    ...
    private function handle_render_page_settings_log_section()
    {
        ...
        echo self::DEBUG_LOG_SETTING_KEY;
        ?>]"><?php 
        \_e('Enable', 'woocommerce_activepayments');
        ?></label>
        <p class="description" id="admin-email-description">
			<?php 
        echo \sprintf(\__('Writes error log to %s.', 'woocommerce_activepayments'), '<a target="_blank" href="' . \content_url('uploads/wpdesk-logs/wpdesk_debug.log') . '">' . \content_url('uploads/wpdesk-logs/wpdesk_debug.log') . '</a>');
        ?>
        </p>
		<?php 
    }
    ...
    {
        $logger_factory = new \ActivePaymentsVendor\WPDesk\Logger\WPDeskLoggerFactory();
        ...
        if (\property_exists($logger_factory, 'shouldLoggerBeActivated')) {
            ...
            $this->logger = new \ActivePaymentsVendor\Monolog\Logger('fallback');
        }
    }
    private function maybe_show_logging_notice()
    {
        ...
        if ($debug_log_enabled) {
            if (\apply_filters('wpdesk_helper_show_log_notices_library', \true)) {
                new \ActivePaymentsVendor\WPDesk\Notice\Notice(\sprintf(
                    // Translators: link.
                    \__('WP Desk Debug Log is enabled. %1$sPlease disable it after testing%2$s.', 'woocommerce_activepayments'),
                    '<a href="' . \admin_url('admin.php?page=wpdesk-helper-settings') . '">',
                    '</a>'
                ), \ActivePaymentsVendor\WPDesk\Notice\Notice::NOTICE_TYPE_INFO);
            }
        }
    }

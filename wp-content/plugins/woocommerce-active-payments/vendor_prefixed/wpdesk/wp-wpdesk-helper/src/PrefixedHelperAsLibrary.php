...
namespace ActivePaymentsVendor\WPDesk\Helper;

use Psr\Log\LoggerInterface;
use ActivePaymentsVendor\WPDesk\Helper\Debug\LibraryDebug;
use ActivePaymentsVendor\WPDesk\Helper\Integration\LicenseIntegration;
use ActivePaymentsVendor\WPDesk\Helper\Integration\LogsIntegration;
use ActivePaymentsVendor\WPDesk\Helper\Integration\SettingsIntegration;
use ActivePaymentsVendor\WPDesk\Helper\Integration\TrackerIntegration;
use ActivePaymentsVendor\WPDesk\Helper\Logs\LibraryInfoProcessor;
use ActivePaymentsVendor\WPDesk\Helper\Page\LibraryDebugPage;
 ...
 */
class PrefixedHelperAsLibrary implements \ActivePaymentsVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    const LIBRARY_TEXT_DOMAIN = 'woocommerce_activepayments';
    ...
    {
        $helper_info = new \ActivePaymentsVendor\WPDesk\Helper\HelperRemoveInfo();
    ...
    private function initialize()
    {
        ...
        $subscription_integration = new \ActivePaymentsVendor\WPDesk\Helper\Integration\LicenseIntegration();
        $subscription_integration->hooks();
        $settingsPage = new \ActivePaymentsVendor\WPDesk\Helper\Page\SettingsPage();
        $settings_integration = new \ActivePaymentsVendor\WPDesk\Helper\Integration\SettingsIntegration($settingsPage);
        $tracker_integration = new \ActivePaymentsVendor\WPDesk\Helper\Integration\TrackerIntegration($settingsPage);
        $logger_integration = new \ActivePaymentsVendor\WPDesk\Helper\Integration\LogsIntegration($settingsPage);
        ...
        $library_debug_info = new \ActivePaymentsVendor\WPDesk\Helper\Debug\LibraryDebug();
        (new \ActivePaymentsVendor\WPDesk\Helper\Page\LibraryDebugPage($library_debug_info))->hooks();
        self::$logger->pushProcessor(new \ActivePaymentsVendor\WPDesk\Helper\Logs\LibraryInfoProcessor($library_debug_info));

...
if (!\class_exists('ActivePaymentsVendor\\WPDesk_Logger')) {
    /**
    ...
    class WPDesk_Logger
    {
        ...
        {
            if (!self::$logger) {
                $loggerFactroy = new \ActivePaymentsVendor\WPDesk\Logger\WPDeskLoggerFactory();
        ...
        {
            if (\defined('ActivePaymentsVendor\\WC_VERSION')) {
                if (\version_compare(\ActivePaymentsVendor\WC_VERSION, '3.0', '<')) {

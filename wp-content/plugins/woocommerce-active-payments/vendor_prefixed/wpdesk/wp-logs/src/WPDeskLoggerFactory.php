...
use LogicException;
use ActivePaymentsVendor\Monolog\Handler\NullHandler;
use ActivePaymentsVendor\Monolog\Logger;
use ActivePaymentsVendor\Monolog\Registry;
use ActivePaymentsVendor\Monolog\Handler\StreamHandler;
use Psr\Log\LogLevel;
use ActivePaymentsVendor\WPDesk\Logger\WC\WooCommerceCapture;
use ActivePaymentsVendor\WPDesk\Logger\WP\WPCapture;
/**
 * Manages and facilitates creation of logger
 *
 * @package WPDesk\Logger
 */
class WPDeskLoggerFactory extends \ActivePaymentsVendor\WPDesk\Logger\BasicLoggerFactory
{
    ...
    public static function tearDown($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (\ActivePaymentsVendor\Monolog\Registry::hasLogger($name)) {
            \ActivePaymentsVendor\Monolog\Registry::removeLogger($name);
        }
    }
    /**
    ...
    public function disableLog($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        if (\ActivePaymentsVendor\Monolog\Registry::hasLogger($name)) {
            /** @var Logger $logger */
            $logger = \ActivePaymentsVendor\Monolog\Registry::getInstance($name);
    ...
    public function createWPDeskLogger($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (!self::$shouldLoggerBeActivated) {
            return new \ActivePaymentsVendor\Monolog\Logger($name);
        }
        if (\ActivePaymentsVendor\Monolog\Registry::hasLogger($name)) {
    ...
    private function captureWPLog()
    {
        static $wpCapture;
        if (!$wpCapture) {
            $wpCapture = new \ActivePaymentsVendor\WPDesk\Logger\WP\WPCapture(\basename($this->getFileName()));
     ...
     * @param Logger $logger
     */
    private function captureWooCommerce(\ActivePaymentsVendor\Monolog\Logger $logger)
    {
        if (!\defined('ActivePaymentsVendor\\WC_LOG_THRESHOLD')) {
            \define('ActivePaymentsVendor\\WC_LOG_THRESHOLD', self::LEVEL_WC);
        }
        $wcIntegration = new \ActivePaymentsVendor\WPDesk\Logger\WC\WooCommerceCapture($logger);
     ...
     */
    private function pushFileHandle($filename, \ActivePaymentsVendor\Monolog\Logger $logger)
    {
        $logger->pushHandler(new \ActivePaymentsVendor\Monolog\Handler\StreamHandler($filename, self::LEVEL_WPDESK_FILE));
    }
    /**
    ...
    public function getFileName($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        return \trailingslashit(\untrailingslashit($upload_dir['basedir'])) . \ActivePaymentsVendor\WPDesk\Logger\WP\WPCapture::LOG_DIR . \DIRECTORY_SEPARATOR . $name . '_debug.log';
    }
    /**
     ...
     * @return void
     */
    private function removeAllHandlers(\ActivePaymentsVendor\Monolog\Logger $logger)
    {
        try {
            while (\true) {
                $logger->popHandler();
            ...
            $logger->pushHandler(new \ActivePaymentsVendor\Monolog\Handler\NullHandler());
        }
    }
    /**
    ...
    {
        return \ActivePaymentsVendor\Monolog\Registry::hasLogger($name);
    }
}

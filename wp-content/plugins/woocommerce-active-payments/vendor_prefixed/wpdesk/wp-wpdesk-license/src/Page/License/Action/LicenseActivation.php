...
use ActivePaymentsVendor\WPDesk\License\ServerAddressRepository;
...
class LicenseActivation implements \ActivePaymentsVendor\WPDesk\License\Page\Action
{
    ...
    public function execute(array $plugin)
    {
        ...
        $this->activate_license($activation_email, $api_key, new \ActivePaymentsVendor\WPDesk\License\ServerAddressRepository($product_id));
    }
    /**
    ...
    {
        \add_settings_error('api_key_check_text', 'api_key_check_error', \__('Connection failed to the Subscription Key API server. Try again later.', 'woocommerce_activepayments'), 'error');
    }
    /**
    ...
    {
        \add_settings_error('activate_text', 'activate_msg', \__('Plugin activated. ', 'woocommerce_activepayments') . "{$activate_results['message']}.", 'updated');
    }
    /**
     * Activate license.
     *
     ...
     */
    public function activate_license($activation_email, $api_key, \ActivePaymentsVendor\WPDesk\License\ServerAddressRepository $address_repository)
    {

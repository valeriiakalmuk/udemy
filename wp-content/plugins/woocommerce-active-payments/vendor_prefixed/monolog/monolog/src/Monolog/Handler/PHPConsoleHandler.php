...
use Exception;
use ActivePaymentsVendor\Monolog\Formatter\LineFormatter;
use ActivePaymentsVendor\Monolog\Logger;
use ActivePaymentsVendor\Monolog\Utils;
use ActivePaymentsVendor\PhpConsole\Connector;
use ActivePaymentsVendor\PhpConsole\Handler;
use ActivePaymentsVendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \ActivePaymentsVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \ActivePaymentsVendor\PhpConsole\Connector $connector = null, $level = \ActivePaymentsVendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('ActivePaymentsVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\ActivePaymentsVendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \ActivePaymentsVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \ActivePaymentsVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\ActivePaymentsVendor\PhpConsole\Helper::isRegistered()) {
            \ActivePaymentsVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \ActivePaymentsVendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \ActivePaymentsVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \ActivePaymentsVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \ActivePaymentsVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}

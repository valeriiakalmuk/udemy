...
use WGPayuVendor\PhpConsole\Connector;
use WGPayuVendor\PhpConsole\Handler;
use WGPayuVendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \WGPayuVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \WGPayuVendor\PhpConsole\Connector $connector = null, $level = \WGPayuVendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('WGPayuVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\WGPayuVendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \WGPayuVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \WGPayuVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\WGPayuVendor\PhpConsole\Helper::isRegistered()) {
            \WGPayuVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \WGPayuVendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \WGPayuVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \WGPayuVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \WGPayuVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}

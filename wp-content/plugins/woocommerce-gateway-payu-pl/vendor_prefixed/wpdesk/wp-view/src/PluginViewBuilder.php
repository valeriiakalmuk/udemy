...
namespace WGPayuVendor\WPDesk\View;

use WGPayuVendor\WPDesk\View\Renderer\SimplePhpRenderer;
use WGPayuVendor\WPDesk\View\Resolver\ChainResolver;
use WGPayuVendor\WPDesk\View\Resolver\DirResolver;
use WGPayuVendor\WPDesk\View\Resolver\WPThemeResolver;
/**
...
class PluginViewBuilder
{
    ...
    {
        $resolver = new \WGPayuVendor\WPDesk\View\Resolver\ChainResolver();
        $resolver->appendResolver(new \WGPayuVendor\WPDesk\View\Resolver\WPThemeResolver(\basename($this->plugin_dir)));
        foreach ($this->template_dirs as $dir) {
            $dir = \trailingslashit($this->plugin_dir) . \trailingslashit($dir);
            $resolver->appendResolver(new \WGPayuVendor\WPDesk\View\Resolver\DirResolver($dir));
        }
        return new \WGPayuVendor\WPDesk\View\Renderer\SimplePhpRenderer($resolver);
    }
    /**

...
use WGPayuVendor\Monolog\Registry;
use WGPayuVendor\Monolog\Handler\StreamHandler;
use Psr\Log\LogLevel;
use WGPayuVendor\WPDesk\Logger\WC\WooCommerceCapture;
use WGPayuVendor\WPDesk\Logger\WP\WPCapture;
/**
 * Manages and facilitates creation of logger
 *
 * @package WPDesk\Logger
 */
class WPDeskLoggerFactory extends \WGPayuVendor\WPDesk\Logger\BasicLoggerFactory
{
    ...
    {
        if (\WGPayuVendor\Monolog\Registry::hasLogger($name)) {
            \WGPayuVendor\Monolog\Registry::removeLogger($name);
        }
    }
    /**
    ...
    public function disableLog($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        if (\WGPayuVendor\Monolog\Registry::hasLogger($name)) {
            /** @var Logger $logger */
            $logger = \WGPayuVendor\Monolog\Registry::getInstance($name);
    ...
    public function createWPDeskLogger($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (!self::$shouldLoggerBeActivated) {
            return new \WGPayuVendor\Monolog\Logger($name);
        }
        if (\WGPayuVendor\Monolog\Registry::hasLogger($name)) {
            return \WGPayuVendor\Monolog\Registry::getInstance($name);
        }
    ...
    private function captureWPLog()
    {
        static $wpCapture;
        if (!$wpCapture) {
            $wpCapture = new \WGPayuVendor\WPDesk\Logger\WP\WPCapture(\basename($this->getFileName()));
     ...
     * @param Logger $logger
     */
    private function captureWooCommerce(\WGPayuVendor\Monolog\Logger $logger)
    {
        if (!\defined('WGPayuVendor\\WC_LOG_THRESHOLD')) {
            \define('WGPayuVendor\\WC_LOG_THRESHOLD', self::LEVEL_WC);
        }
        $wcIntegration = new \WGPayuVendor\WPDesk\Logger\WC\WooCommerceCapture($logger);
     ...
     */
    private function pushFileHandle($filename, \WGPayuVendor\Monolog\Logger $logger)
    {
        $logger->pushHandler(new \WGPayuVendor\Monolog\Handler\StreamHandler($filename, self::LEVEL_WPDESK_FILE));
    }
    /**
    ...
    public function getFileName($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        return \trailingslashit(\untrailingslashit($upload_dir['basedir'])) . \WGPayuVendor\WPDesk\Logger\WP\WPCapture::LOG_DIR . \DIRECTORY_SEPARATOR . $name . '_debug.log';
    }
    /**
     ...
     * @return void
     */
    private function removeAllHandlers(\WGPayuVendor\Monolog\Logger $logger)
    {
        try {
            while (\true) {
                $logger->popHandler();
            ...
            $logger->pushHandler(new \WGPayuVendor\Monolog\Handler\NullHandler());
        }
    }
    /**
    ...
    {
        return \WGPayuVendor\Monolog\Registry::hasLogger($name);
    }
}

...
use WGPayuVendor\WPDesk\License\InstalledPlugins;
/**
 * Can render and manage license page.
 *
 * @package WPDesk\License\Page\License
 */
class LicensePage implements \WGPayuVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    public function __construct(\WGPayuVendor\WPDesk\License\InstalledPlugins $plugin_database)
    {
    ...
    public function handle_render_wpdesk_licenses_page()
    {
        ...
        if (!\class_exists('WGPayuVendor\\WPDesk_Helper_List_Table')) {
    ...
    private function execute_plugin_action($plugin, $action)
    {
        ...
        if (isset($plugin_info)) {
            (new \WGPayuVendor\WPDesk\License\Page\LicensePageActions())->create_action($action)->execute($plugin_info);
        }
    }
    /**
